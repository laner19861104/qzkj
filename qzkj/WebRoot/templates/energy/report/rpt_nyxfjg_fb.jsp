...
//String countStr="";
//if(resMap.get("resCountJson")!=null)//能耗结构数据
 //   countStr=resMap.get("resCountJson").toString();
...
//var countListLoop=;
function loadCell1(){
   url1="${path}/cellTemplate/nyxfjg_fb_new.cll";
   cell.OpenFile(url1,"");    
   var i=0; 
   var pand=true;
   var startrow=0;
  ...
  //统计负责人
   ...
   for(i=0;i<printListLoop.length;i++)
   {
      ...
      {
        if(cell_dic[item]==null || cell_dic[item]=='nd')
             continue;
	...
	<tr>
	  <td>
			<table border="0" cellpadding="0" cellspacing="0" class="Menubox">
			  <tr>
				<td><ul>
					<li id="one1" ><span style="font-size:12px;"><a href="${path}/consumptionStructurEnyRpt?zbid=<%=sbinfoMap.get("zbid")%>&dwbh=<%=resMap.get("dwbh")%>&nd=<%=resMap.get("nd")%>&yd=<%=resMap.get("yd")%>" target="_self">能源消费结构表</a></span></li>
				  	<li id="one2" class="hover"><span style="font-size:12px;">能源消费结构表(附表)</span></li>
				</ul></td>
			  </tr>
			</table>
