...
function loadCell1(){
   url1="${path}/cellTemplate/zynhsbzkb_ry.cll";
   cell.OpenFile(url1,"");    
   var i=0; 
   var pand=true;
   var startrow=0;
  ...
  //  alert("--"+col1+","+row1);
   ...
   cell.InsertRow(5, printListLoop.length-1,0);
			  ...
			  <tr>
				<td><ul>
					<li id="one1" ><span style="font-size:12px;"><a href="${path}/facilityStatusEnyRpt?zbid=<%=sbinfoMap.get("zbid")%>&dwbh=<%=resMap.get("dwbh")%>&nd=<%=resMap.get("nd")%>&yd=<%=resMap.get("yd")%>" target="_self">主要能耗设备状况表</a></span></li>
				  	<li id="one2" ><span style="font-size:12px;"><a href="${path}/facilityStatusSBGLEnyRpt?zbid=<%=sbinfoMap.get("zbid")%>&dwbh=<%=resMap.get("dwbh")%>&nd=<%=resMap.get("nd")%>&yd=<%=resMap.get("yd")%>" target="_self">企业能源计量器具配备和管理情况表</a></span></li>
				  	<li id="one2" class="hover"><span style="font-size:12px;">操作人员培训情况汇总表</span></li>
				</ul></td>
			  </tr>
			</table>
