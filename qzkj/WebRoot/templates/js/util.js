...
function checkyear(year) {
 if (!(check(year.value, "1234567890")&&(year.value.length==4))) {
		alert("年份输入有误，请输入数字!");
		year.value = "";
		year.focus();
		bcontrol = false;
...
function generateDate(x, y, z) {
	...
	//判断闰年
	var dayfor2 = 28;
	if ((x.value % 100 != 0 && x.value % 4 == 0)
			|| (x.value % 100 == 0 && x.value % 400 == 0))
		dayfor2 = 29;
	//alert(dayfor2);
	temp = y.length - 12;
	...
	//大月
	if ((y.value < 8 && y.value % 2 == 1) || (y.value > 7 && y.value % 2 == 0))
		dayOfMonth = 31;
...
输入id和选择的name
返回将对应的option选择
Add by syl 2010-11-01
*/
     

function selsectChect(id,name){
var s_optionsid = document.getElementById(name);
for(var i=0;i<s_optionsid.length;i++){
 if(id==s_optionsid[i].value){
   s_optionsid[i].selected=true;
}
}
}
...
num 数字
type 0 返回小写字母，1为返回大些字母
**/
function numberToAbc(num,type){
    var abc;
  //  alert(num);
  //  alert(type);
  switch (num){
