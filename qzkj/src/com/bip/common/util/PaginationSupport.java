...
public class PaginationSupport {
	...
	public String getPagefooter_T() {
		...
		if (this.getViewPrevious()) {
			str.append(
				"<INPUT  type=submit value=首页 name=firs onclick=\"this.form.currentPage.value=1\"  >");
		} else {
			str.append("<INPUT   type=submit value=首页 name=firs  disabled>");
		}
		if (this.getViewPrevious()) {
			str.append(
				"<INPUT  type=submit value=上页 name=prev onclick=\"this.form.currentPage.value='");
		...
		if (this.getPageCount() > 1 && this.getViewNext()) {
			str.append(
				"<INPUT  type=submit value=下页 name=next onclick=\"this.form.currentPage.value='");
		...
		if (this.getPageCount() > 1 && this.getViewNext()) {
			str.append(
				"<INPUT   type=submit value=末页 name=last onclick=\"this.form.currentPage.value='");
	...
	public String getPagefooter() {
		...
		if(this.getViewPrevious()){
			...
			str.append("&nbsp;<a href=\"javascript:formlist.submit();\"  onclick=\"formlist.currentPage.value='").append(this.getCurrentPage()-1).append("'\" >上一页</a>");
		}else{
			str.append("首 页  上一页 ");
		}
		if(this.getViewNext()){
			str.append("&nbsp;<a    onclick=\"formlist.currentPage.value=").append(this.getCurrentPage()+1).append("\"   href=\"javascript:formlist.submit();\">下一页</a> ");
			str.append("&nbsp;<a  onclick=\"formlist.currentPage.value='").append(this.getPageCount()).append("'\"  href=\"javascript:formlist.submit();\">末 页</a>");
		}else{
			str.append(" 下一页  末 页");
		}
		str.append(" 转到");
		...
		str.append("【第<span>").append(this.getCurrentPage()).append(" </span>页 , 共<span>");
		str.append(this.getPageCount()).append(" </span>页 】【<span>").append(this.getPageSize()).append("</span>条/页 , 共<span>").append(this.getTotalCount()).append("</span>条记录】");
		str.append(
		"<INPUT type=hidden  value=").append(this.getCurrentPage()).append(" name=\"currentPage\" id=\"currentPage\"> ");
		}
		return str.toString();
