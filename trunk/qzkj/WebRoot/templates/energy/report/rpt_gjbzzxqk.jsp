...
function loadCell1(){
   url1="${path}/cellTemplate/gjbzzxqk.cll";
   cell.OpenFile(url1,"");    
   var i=0; 
   var pand=true;
   var startrow=0;
  ...
  //  alert("--"+col1+","+row1);
   ...
   for(i=0;i<printListLoop.length;i++)
   {
      ...
      if(cell_dic1.count>1){
         //alert("--"+(row1+i+nr1-1)+","+(row1+i+nr1-1+cell_dic1["count"]-1));
     ...
     // nr2+=nrs;
     // alert("nrs==="+nrs+"====nr2="+nr2);
      for(j=0;j<printList.length;j++){
          var cell_dic2=printList[j];
          getCellVar("khzbzx");
        //  alert("nr2="+nr2)
          if(cell_dic2["count"]>1){
          	// alert("nrs="+nrs+"==nr2="+nr2+"=j="+j+"=khzbzx--"+(row1+j+nr2-1)+","+(row1+j+nr2-1+cell_dic2["count"]-1));
	         //j+nr2-1
     ...
     //     nr2+=cell_dic2.count-1+nrs;
          ...
          for(k=0;k<pList.length;k++){
		     ...
		     {
		       getCellVar(item);
		       cell.MergeCells(3,row1+ii,4,row1+ii);
		       if(isDefined);
		         {
