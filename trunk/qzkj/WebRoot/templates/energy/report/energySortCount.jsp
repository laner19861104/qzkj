...
String lbStr="";
if(resMap.get("lbJson")!=null)//能耗结构数据
...
function init()
{
  ...
  // alert(printListLoop.length)
   ...
   //	 alert("==");
   	  ...
   	  //  alert("=="+ll);
   	 // alert("列="+(2+i)+"; 行="+(printListLoop.length+3)+"; 公式="+"SUM("+ll+"3:"+ll+(printListLoop.length+2)+")");
   	  cell.SetFormula(2+i,printListLoop.length+3,0,"SUM("+ll+"3:"+ll+(printListLoop.length+2)+")");
   }
   var lz=numberToAbc(2+lbListLoop.length,1);
   
 //  alert("列="+(2+lbListLoop.length)+"; 行="+(printListLoop.length+3)+"; 公式="+"SUM("+lz+"3:"+lz+(printListLoop.length+2)+")");
   ...
   for(i=0;i<printListLoop.length;i++)
   {
     ...
     // alert("列="+(2+lbListLoop.length)+"; 行="+(i+3)+"; 公式="+"SUM("+lc+(i+3)+":"+lr+(i+3)+")");
...
<body onload=init()>
	<div class="position">当前位置：能源消耗数据库系统 &gt;&gt; <a href="#">能源消耗数据分类汇总</a></div>
