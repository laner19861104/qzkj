...
<%@ page import="com.bip.common.util.Tool"%>
...
String sbrq="";
//out.print(sbinfoMap.get("sbrq"));
if(sbinfoMap.get("sbrq")!=null){
    Date dl=Tool.stringToDate(sbinfoMap.get("sbrq").toString(),"yyyy-mm-dd");
 	sbrq=Tool.yearOfDate(dl)+"年"+Tool.monthOfDate(dl)+"月（季度）";    
}
%>
...
function init()
{
  if(cell.Login("Beelink software", "","13040483", "7140-0421-0057-6004")==0)
   ...
   url1="${path}/cellTemplate/qyystjb.cll";
   cell.OpenFile(url1,"");    
   var i=0; 
   var pand=true;
   var startrow=0;
   ...
   cell.S(col1,row1,0,'<%=resMap.get("dwmc")%>');
    ...
    getCellVar('theYear');
   cell.S(col1,row1,0,'<%=sbrq%>');
   
   getCellVar('fr');
  // alert("--"+col1+","+row1);
   cell.S(col1,row1,0,"<%=resMap.get("fr")%>");
