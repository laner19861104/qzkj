...
String resStr1="";
if(resMap.get("resJson1")!=null)//进出用能单位
    resStr1=resMap.get("resJson1").toString();
String resStr2="";
if(resMap.get("resJson2")!=null)//次级进出用能单位
    resStr2=resMap.get("resJson2").toString();
String resStr3="";
if(resMap.get("resJson3")!=null)//主要用能设备
    resStr3=resMap.get("resJson3").toString();
    
String resXj="";    
if(resMap.get("xjJson")!=null)//小计
    resXj=resMap.get("xjJson").toString();
    

String resXm="";    
if(resMap.get("xmJson")!=null)//项目及要求
		        ...
		        if(isDefined);
		          {
		            ...
		            if(item=="sfbz"){
		           		cell.S(col1,row1+ii,0,cell_dic[item]==0 ? "否":"是");
		            }else{
        ...
        if(isDefined);
          {
            //alert(item+"--"+col1+","+(row1+i)+";value="+cell_dic[item]);
            if(item=="sfzqn"){
		        cell.S(col1,row1+i,0,cell_dic[item]==0 ? "否":"是");
		    }else{	
	            if(item.indexOf('sum')!=-1 || typeof cell_dic[item] == 'number')
	...
	<tr>
	  <td>
			<table border="0" cellpadding="0" cellspacing="0" class="Menubox">
			  <tr>
				<td><ul>
					<li id="one1" ><span style="font-size:12px;"><a href="${path}/facilityStatusEnyRpt?zbid=<%=sbinfoMap.get("zbid")%>&dwbh=<%=resMap.get("dwbh")%>&nd=<%=resMap.get("nd")%>&yd=<%=resMap.get("yd")%>" target="_self">主要能耗设备状况表</a></span></li>
				  	<li id="one2" class="hover"><span style="font-size:12px;">企业能源计量器具配备和管理情况表</span></li>
				  	<li id="one2" ><span style="font-size:12px;"><a href="${path}/facilityStatusRYEnyRpt?zbid=<%=sbinfoMap.get("zbid")%>&dwbh=<%=resMap.get("dwbh")%>&nd=<%=resMap.get("nd")%>&yd=<%=resMap.get("yd")%>" target="_self">操作人员培训情况汇总表</a></span></li>
				</ul></td>
			  </tr>
			</table>
