...
WebFXCheckBoxTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var tempStr = "<input type=\"checkbox\"" + " class=\"tree-check-box\"" +
		...
		" value=\"" + this.value + "\" id=\"" + this.id + "-input\" name=\"" + this.value + "-input\" " + (this._disabled?" disabled ": "") + ">";
...
//设置全部子节点为未选中状态，当父节点未被选中时
function unCheckchilds(item){
      var cnodes=item.childNodes;
		for(var i=0; i<cnodes.length; i++){
          var cbx = getCheckBox(cnodes[i].id);
		  cbx.checked=false;
          unCheckchilds(cnodes[i])
		}
}
// 循环当前点击节点的所有子节点.
